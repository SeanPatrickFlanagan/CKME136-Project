...
for (i in 1:(num_cat_vars - 1)){
  for (j in (i + 1) : num_cat_vars) {
    gtest <- gchi2Test(cat_vars[,i], cat_vars[,j])
    if (gtest[2,2] <= 0.05) {
      print (paste(names(cat_vars[i]), " and ", names(cat_vars[j]), " are not independent.", format(gtest[2,2], digits = 4)))
    } else {
      print (paste(names(cat_vars[i]), " and ", names(cat_vars[j]), " are independent.", format(gtest[2,2], digits = 4)))
    }
  } 
}
...
```{r}
# Create a categorical only dataset. Discretize continuous variables using interval = square root of the range and convert to factors
...
# use FSelector information gain to determine most important attributes based on information gain
...
```{r}
#Split the data into training (80%) and test set (20%). Further split the training set in training and validation sets. Consider possibly treatment for imbalance in the target variable, readmitted.

#Partition the dataset into training and test sets (use the discretized dataframe)
set.seed(136)
trainIndex <- createDataPartition(readmission_all_cat$readmitted, p = .8, 
                                  list = FALSE, 
                                  times = 1)
readmitted_train <- readmission_all_cat[trainIndex,] 
readmitted_test <- readmission_all_cat[-trainIndex,] 

#further split training into training and validation
trainIndex2 <- createDataPartition(readmitted_train$readmitted, p = .8, 
                                  list = FALSE, 
                                  times = 1)
readmitted_train2 <- readmitted_train[trainIndex2,]
readmitted_valid <- readmitted_train[-trainIndex2,]
```
...
#Loop through all folds of cross validation, compute confusion matrix, keep running total
  for (i in 1:num_folds) {
    ...
    readmitted_train <- readmitted_df[-trainIndex[[i]],] # use all folds except current index for training
    readmitted_test <- readmitted_df[trainIndex[[i]],]   # use current index fold for test

    if (ml_type == "NB") {model1 <- naiveBayes(readmitted ~ ., data = readmitted_train, laplace = 1)} 
    else if (ml_type == "DT") {model1 <- J48(readmitted ~ ., data = readmitted_train)}
    else if (ml_type == "RF") {model1 <- randomForest(readmitted ~ ., data=readmitted_train, ntree=100)}
    else if (ml_type == "SV") {model1 <- svm(readmitted ~ ., data = readmitted_train, kernel = 'linear', gamma = 0.05, cost = 5)}
    
    # predict classes based on trained model for test data    
    preds <- predict(model1, newdata = readmitted_test)
    # generate confusion matrix    
    new_conf_matrix <- table(preds, readmitted_test$readmitted)
...
#
perf_measures <- run_models(readmitted_train2, c("NB", "DT", "RF")) # not using SV because it takes too long
perf_measures
...
reduced_attrib_vector <- c('number_inpatient', 'discharge_disposition_id', 'age', 'number_diagnoses', 'num_lab_procedures', 'time_in_hospital', 'diag_2', 'diabetesMed', 'readmitted')
reduced_df <- readmitted_train2[, reduced_attrib_vector]

perf_measures <- run_models(reduced_df, c("NB", "DT", "RF"))
perf_measures
```
...
# subset the training set further into training and validation sets
readmitted_binary_train2 <- readmitted_binary_train[trainIndex2,]
readmitted_binary_valid <- readmitted_binary_train[-trainIndex2,]

#
# Run the models using attributes determined from feature selection:
#
reduced_binary_df <- readmitted_binary_train2[, reduced_attrib_vector]

perf_measures <- run_models(reduced_binary_df, c("NB", "DT", "RF", "SV"))
perf_measures
```
...
```{r}
# Balance the training dataset by undersampling the majority classes and rerun models to see if that improves accuracy and F1 scores. Models tested using validation set rather than CV.
# Use the number of observations from the smallest class (readmitted <30) as number randomly selected from other classes
n_smallest_class <- length(readmitted_train2$readmitted[readmitted_train2$readmitted=="<30"])
readmitted_balanced <- readmitted_train2 %>% group_by(readmitted) %>% sample_n(n_smallest_class)
#
# Run the models using the balanced dataset on the reduced attribute set
# Use validation set to test effectiveness of trained models on the reduced attribute set for all models

reduced_balanced_train <- readmitted_balanced[, reduced_attrib_vector]
reduced_valid_df <- readmitted_valid[, reduced_attrib_vector]

# Naive Bayes
model2 <- naiveBayes(readmitted ~ ., data = reduced_balanced_train, laplace = 1)
# predict classes based on trained model for test data    
preds <- predict(model2, newdata = reduced_valid_df)
# generate confusion matrix    
conf_matrix <- table(preds, reduced_valid_df$readmitted)
names(dimnames(conf_matrix)) <- c("Predicted Class", "Actual Class")
conf_matrix
perf_measures <- format(calc_perf_meas(conf_matrix), digits = 3)
perf_measures
...
# Decision tree
model2 <- J48(readmitted ~ ., data = reduced_balanced_train)
# predict classes based on trained model for test data    
preds <- predict(model2, newdata = reduced_valid_df)
# generate confusion matrix    
conf_matrix <- table(preds, reduced_valid_df$readmitted)
...
# Random Forest
model2 <- randomForest(readmitted ~ ., data=reduced_balanced_train, ntree=100)
# predict classes based on trained model for test data    
preds <- predict(model2, newdata = reduced_valid_df)
# generate confusion matrix    
conf_matrix <- table(preds, reduced_valid_df$readmitted)
...
# Support Vector Machine
model2 <- svm(readmitted ~ ., data = reduced_balanced_train, kernel = 'linear', gamma = 0.05, cost = 5)
# predict classes based on trained model for test data    
preds <- predict(model2, newdata = reduced_valid_df)
# generate confusion matrix    
conf_matrix <- table(preds, reduced_valid_df$readmitted)
...
```{r}
# Balance class attribute after converting to binary class (combine >30 and NOT classes ... i.e. either readmitted under 30 days or not at all). Run models after balancing.
...
readmission_balanced <- SMOTE(readmitted ~ ., readmission_binary_class, perc.over = o_balance_factor, perc.under = u_balance_factor)
